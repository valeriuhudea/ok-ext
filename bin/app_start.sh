...
if [ "$#" -eq 0 ] || [ $1 = "-h" ] || [ $1 = "--help" ]; then
    echo "Usage: ./vhapp [OPTIONS] COMMAND [arg...]"
    echo "       ./vhapp [ -h | --help ]"
    echo ""
    echo "Options:"
...
stop_existing() {
  vhapp="$(docker ps --all --quiet --filter=name=vhapp)"
  REDIS="$(docker ps --all --quiet --filter=name=vhapp-redis)"
  MONGO="$(docker ps --all --quiet --filter=name=vhapp-mongo)"
